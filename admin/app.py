...
def ai_settings():
    ...
    return render_template('ai.html', config=ai_config)
...
def update_ai_config():
    ...
    # Сохраняем старый ключ если новый пустой
    new_key = request.form.get('api_key', '').strip()
    if new_key:
        ai_config['api_key'] = new_key
    
    ai_config['model'] = request.form.get('model', 'deepseek/deepseek-r1-0528:free').strip()
    ai_config['enabled'] = request.form.get('enabled') == 'on'
    
    config_manager.save(config)
    
    # Обновляем анализатор если он инициализирован
    try:
...
def update_ai_prompt():
    ...
    if prompt:
        ai_config['prompt'] = prompt
    
    config_manager.save(config)
    
    # Обновляем промпт в анализаторе
    try:
        from ai_analyzer import get_analyzer
        analyzer = get_analyzer()
        if analyzer:
            analyzer.update_settings(prompt=prompt)
    except ImportError:
        pass
    
    flash('Промпт обновлен', 'success')
    return redirect(url_for('ai_settings'))
...
def api_ai_test():
    ...
    if not ai_config.get('api_key'):
        return jsonify({'success': False, 'error': 'API ключ не настроен'})
    ...
    try:
        ...
        analyzer = AIAnalyzer(
            api_key=ai_config.get('api_key'),
            model=ai_config.get('model', 'deepseek/deepseek-r1-0528:free'),
        ...
        return jsonify({'success': False, 'error': 'Модуль ai_analyzer не найден'})
